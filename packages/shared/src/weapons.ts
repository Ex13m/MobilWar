...
const HEAVIES: Row[] = [
  ...
  ["Вакуум-Т", "тянет: стан 1 с, сплэш 8", "stun", 25, 2800, 40, 12, 1, 3000, "rocket2", 0xc4b5fd, 0.75, { splashM: 8, damageEdge: 15, fuseM: 3, speedMps: 20, stunMs: 1000 }],
  ...
  ["Резонатор", "цепь + ЭМИ-эффект", "chain", 30, 2800, 40, 10, 1, 3000, "rocket2", 0x5eead4, 0.7, { splashM: 5, damageEdge: 10, fuseM: 3, speedMps: 22 }],
...
export function ttk(w: WeaponDef): number {
  const perTrigger = w.damage * Math.max(1, w.burst) * Math.max(1, w.pellets);
